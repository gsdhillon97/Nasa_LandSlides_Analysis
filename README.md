...
![Hypercars Expo conceptual diagram showing Customers, Transaction Details, Hypercars, and Locations](assets/conceptual-diagram.png)

[Download the editable draw.io diagram](assets/conceptual-diagram.drawio).

This conceptual model presents the project's business entities and relationships. The SQL implementation expands the design into the eight tables listed above; the diagram's attributes, cardinalities, and location-to-vehicle relationship do not map directly to all implemented columns and constraints.

In the SQL implementation, each rental or purchase references one transaction through a unique `transaction_id`. Its own `rental_id` or `purchase_id` is a separate identity primary key. The schema does not require every transaction to have a matching detail record or prevent one transaction from appearing in both detail tables.
